...
This section summarizes the final state of key subsystems after the full cleanup
plan (Phases 1 through 19) was completed.

### Canonical Router

The single production router is built by `build_production_router` in
`src/api/mod.rs`. It applies JWT authentication middleware on all protected
routes and RBAC enforcement middleware for permission-specific routes. The
GraphQL endpoint at `/graphql` requires JWT authentication for execution.

Legacy REST router builders remain available only under `#[cfg(test)]` gates and
are not re-exported as public production entrypoints.

### OIDC Session Storage

OIDC pending sessions are stored by the `OidcSessionStore` trait
(`src/auth/oidc/session_store.rs`). Three backends are available:

- `RedisOidcSessionStore`: production-grade, Redis-backed, uses sorted-set
  indexes for O(1) pending-session counting per principal and global session
  cleanup.
- `InMemoryOidcSessionStore`: development and test use, in-process HashMap with
  RwLock.
- `NullOidcSessionStore`: testing stub that rejects all sessions.

The active backend is selected by `auth.keycloak.session_store.backend` in
`config/default.yaml`. The production setting uses `redis`.

### GraphQL OPA Authorization

Resolver-level authorization is provided by the `GraphqlPolicyEvaluator` trait
(`src/api/graphql/authorization.rs`). The concrete implementation
`OpaClientGraphqlEvaluator` evaluates OPA policies before protected queries and
mutations execute. A fakeable evaluator supports tests without a live OPA
server.

The canonical router injects the GraphQL authorization service when OPA
middleware state is configured.

### Public API Surface

The public crate exports are controlled by `src/lib.rs`. Key stable exports:

- `xzepr::api::build_production_router` and `xzepr::api::RouterConfig`
- `xzepr::domain::entities::*` (event, event_receiver, event_receiver_group,
  user)
- `xzepr::domain::repositories::*` (repository traits)
- `xzepr::domain::value_objects::*` (typed IDs)
- `xzepr::auth::jwt::{JwtService, JwtConfig, Claims}`
- `xzepr::auth::rbac::{Role, Permission}`
- `xzepr::opa::{OpaClient, OpaConfig, OpaFailSafeMode}`
- `xzepr::infrastructure::RateLimitSecurityConfig`
- `xzepr::infrastructure::RedactedSecret`

### API-Key Digest Strategy

API key verification uses SHA-256 hashing. The raw key is hashed before storage
and lookup in `src/infrastructure/database/postgres_api_key_repo.rs`. The digest
strategy is tied to the repository implementation and not currently configurable
at runtime.

### External Integration Test Gates

Optional integration tests are compiled with Cargo features and gated by
environment variables. None run in the default CI suite.

| Suite          | Cargo Feature                | Environment Variable                        | Test File                               |
| -------------- | ---------------------------- | ------------------------------------------- | --------------------------------------- |
| Kafka/Redpanda | `kafka-integration-tests`    | `XZEPR_RUN_KAFKA_INTEGRATION_TESTS=true`    | `tests/kafka_auth_integration_tests.rs` |
| PostgreSQL     | `database-integration-tests` | `XZEPR_RUN_DATABASE_INTEGRATION_TESTS=true` | `tests/database_tests.rs`               |
| Redis          | `redis-integration-tests`    | `XZEPR_RUN_REDIS_INTEGRATION_TESTS=true`    | `tests/redis_integration_tests.rs`      |
| OIDC           | `oidc-integration-tests`     | `XZEPR_RUN_OIDC_INTEGRATION_TESTS=true`     | `tests/oidc_integration_tests.rs`       |
| OPA            | `opa-integration-tests`      | `XZEPR_RUN_OPA_INTEGRATION_TESTS=true`      | `tests/opa_integration_tests.rs`        |
