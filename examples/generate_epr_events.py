...
def debug_except_hook(type, value, tb):
    print(f"XZEPR python hates {type.__name__}")
    print(str(type))
    import pdb
    import traceback
...
debug = os.environ.get("XZEPR_DEBUG", False)
if debug:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate and post XZEPR event_receivers and events, print curl commands (dry-run), or write to disk."
    )
    parser.add_argument(
        "--url",
        "-u",
    ...
    if args.write_to_disk:
        os.makedirs("XZEPR_reports/event_receivers", exist_ok=True)
        for evr in event_receivers:
            evr_type = evr["type"]
            filename = f"XZEPR_reports/event_receivers/{evr_type.replace('.', '_')}.json"
            with open(filename, "w") as f:
                json.dump(evr, f, indent=2)
            print(f"Wrote event receiver {evr_type} to {filename}")
        os.makedirs("XZEPR_reports/events", exist_ok=True)
        for idx, event in enumerate(events, start=1):
            filename = f"XZEPR_reports/events/event_{idx:03d}_{event['name']}_{event['version']}.json"
            with open(filename, "w") as f:
                json.dump(event, f, indent=2)
            print(f"Wrote event {event['payload']['context']['id']} to {filename}")
        print(f"Wrote {len(events)} events to disk in the 'XZEPR_reports/events' directory")
        # Write curl commands
        with open("XZEPR_reports/curl_commands_event_receivers.txt", "w") as f:
            for evr in event_receivers:
                curl = make_curl_command(evr, f"{url}/api/v1/receivers")
                f.write(curl + "\n")
        with open("XZEPR_reports/curl_commands_events.txt", "w") as f:
            for event in events:
                curl = make_curl_command(event, f"{url}/api/v1/events")
                f.write(curl + "\n")
        print("Wrote curl commands to XZEPR_reports/curl_commands_event_receivers.txt and XZEPR_reports/curl_commands_events.txt")
    if args.dry_run:
